...
using OsuDatabaseControl.Filter.Enums.Parser;
...
public class FilterParser
{
    private static readonly Regex query_syntax_regex = new Regex(
        ...
        /// </summary>
        /// <param name="textFilter">The <see cref="OptionalTextFilter"/> to store the parsed data into, if successful.</param>
        /// <param name="op">
        ...
        public static bool TryUpdateCriteriaText(ref OptionalTextFilter textFilter, Operator op, string value)
        {
            switch (op)
            {
                case Operator.Equal:
        ...
        /// </summary>
        /// <param name="range">
        /// The <see cref="float"/>-typed <see cref="OptionalRange{T}"/>
        ...
        public static bool TryUpdateCriteriaRange(ref OptionalRange<float> range, Operator op, string val, float tolerance = 0.05f)
            => tryParseFloatWithPoint(val, out float value) && tryUpdateCriteriaRange(ref range, op, value, tolerance);

        private static bool tryUpdateCriteriaRange(ref OptionalRange<float> range, Operator op, float value, float tolerance = 0.05f)
        {
            switch (op)
            {
                default:
        ...
        /// </summary>
        /// <param name="range">
        /// The <see cref="double"/>-typed <see cref="OptionalRange{T}"/>
        /// to store the parsed data into, if successful.
        /// </param>
        /// <param name="op">The operator for the keyword filter.</param>
        /// <param name="val">The value of the keyword filter.</param>
        /// <param name="tolerance">Allowed tolerance of the parsed range boundary value.</param>
        public static bool TryUpdateCriteriaRange(ref OptionalRange<double> range, Operator op, string val, double tolerance = 0.05)
            => tryParseDoubleWithPoint(val, out double value) && tryUpdateCriteriaRange(ref range, op, value, tolerance);

        private static bool tryUpdateCriteriaRange(ref OptionalRange<double> range, Operator op, double value, double tolerance = 0.05)
        {
            switch (op)
            {
                default:
        ...
        /// and the resulting range constraint is stored into <paramref name="range"/>.
        /// </summary>
        /// <param name="range">The <see cref="OptionalRange{T}"/> to store the parsed data into, if successful.</param>
        /// <param name="op">The operator for the keyword filter.</param>
        /// <param name="val">The value of the keyword filter.</param>
        ...
        public static bool TryUpdateCriteriaRange<T>(ref OptionalRange<T> range, Operator op, string val, TryParseFunction<T> parseFunction)
            where T : struct
        ...
        /// </summary>
        /// <param name="range">The <see cref="OptionalSet{T}"/> to store the parsed data into, if successful.</param>
        /// <param name="op">The operator for the keyword filter.</param>
        /// <param name="filterValue">The value of the keyword filter.</param>
        public static bool TryUpdateCriteriaSet<T>(ref OptionalSet<T> range, Operator op, string filterValue)
            where T : struct, Enum
        {
        ...
        private static bool tryUpdateCriteriaRange<T>(ref OptionalRange<T> range, Operator op, T value)
            where T : struct
        {
            switch (op)
            {
        ...
        /// </summary>
        private static bool tryUpdateDateAgoRange(ref OptionalRange<DateTimeOffset> dateRange, Operator op, string val)
        {
            switch (op)
            {
                case Operator.Equal:
